...
alter table public.profiles add column if not exists role_text text;
-- 2. Copy values (only for rows where role_text is null)
update public.profiles set role_text = role::text where role_text is null;
...
DO $$ BEGIN
  -- If no columns still use the enum, drop dependent functions then the type
  IF NOT EXISTS (
    ...
    -- Drop function set_user_role if it still references enum
    IF EXISTS (
