...
type ToastContextType = {
    ...
    warning: (message: string, opts?: Omit<ToastItem, 'id' | 'message' | 'variant'>) => string
}
...
export function ToastProvider({ children, maxVisible = 3, dedupeWindowMs = 500, allowActions = true }: ToastProviderProps) {
    ...
    const remove = useCallback((id: string) => {
        ...
        if (queue.current.length > 0) {
            ...
            timeouts.current[next.id] = window.setTimeout(() => remove(next.id), remaining.current[next.id])
        }
    }, [])
    ...
    const success = useCallback((message: string, opts?: Omit<ToastItem, 'id' | 'message' | 'variant'>) => push({ message, variant: 'success', ...(opts || {}) }), [push])
    const error = useCallback((message: string, opts?: Omit<ToastItem, 'id' | 'message' | 'variant'>) => push({ message, variant: 'error', ...(opts || {}) }), [push])
    const info = useCallback((message: string, opts?: Omit<ToastItem, 'id' | 'message' | 'variant'>) => push({ message, variant: 'info', ...(opts || {}) }), [push])
    const warning = useCallback((message: string, opts?: Omit<ToastItem, 'id' | 'message' | 'variant'>) => push({ message, variant: 'warning', ...(opts || {}) }), [push])

    const value = useMemo(() => ({ push, remove, clearAll, success, error, info, warning }), [push, remove, clearAll, success, error, info, warning])
                    ...
                    <div
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Escape') {
                                ...
                                e.stopPropagation()
                                remove(t.id)
                            } else if ((e.key === 'Enter' || e.key === ' ') && t.action && e.currentTarget === e.target) {
                        ...
                        onMouseEnter={() => {
                            ...
                            if (to) {
                                window.clearTimeout(to)
                                const elapsed = Date.now() - (lastStart.current[id] || Date.now())
                                remaining.current[id] = Math.max(0, (remaining.current[id] || t.duration || 0) - elapsed)
                                delete timeouts.current[id]
                            }
                        }}
                        onMouseLeave={() => {
