...
export function RealEstateContent() {
  ...
  const fetchRealEstateContent = useCallback(
    async (showToast = false) => {
      try {
        ...
        console.log(
          "Real estate content fetched successfully:",
          data?.length || 0,
          "records",
        );
        ...
        if (showToast) {
          push({
            title: "Refreshed",
            message: "Real estate content updated",
            variant: "success",
          });
        }
      } catch (error) {
        console.error(
          "Error fetching real estate content from Supabase:",
          error,
        );
        setError(
          error instanceof Error
            ? error.message
            : "Failed to fetch real estate content",
        );
        setRealEstateData([]);
        if (showToast) {
          push({
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    try {
      ...
      if (first) {
        ...
        push({
          title: "URL captured",
          message: "Dropped URL ready to generate.",
          variant: "success",
        });
      } else {
        push({
          title: "No URL found",
          message: "Drop a valid URL (starting with http/https).",
          variant: "warning",
        });
      }
    } catch (err) {
    ...
    <PageContainer>
      <PageHeader
        ...
        actions={
          <>
            <Button
              onClick={() => fetchRealEstateContent(true)}
              loading={loading}
              disabled={loading}
              variant="outline"
            >
