  ...
  const handleViewStrategy = (strategy: Strategy, company: Company) => {
    ...
    void (async () => {
      try {
        ...
        if (error) {
          console.warn(
            "Unable to fetch existing ideas for strategy",
            strategy.id,
            error,
          );
          return;
        ...
        const uniqueSorted = Array.from(new Set<number>(usedAngles)).sort(
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-6 min-h-0">
              ...
              <div className="bg-gradient-to-r from-brand-50 to-purple-50 rounded-xl p-4 border border-brand-100">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">
                      {getPlatformBadges(viewModal.strategy.platforms).join(
                        ", ",
                      )}
                    </p>
                  </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {getAnglesFromStrategy(viewModal.strategy).map(
                    (angle, index) => (
                      <div
                        key={index}
                        ...
                        onClick={() => {
                          if (editingAngle !== angle.number)
                            toggleAngle(angle.number);
                        }}
                        role="button"
                        tabIndex={0}
