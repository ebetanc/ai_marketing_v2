...
export function useAsync<T = unknown>() {
    ...
    const run = useCallback(
        async <R = T>(task: Promise<R> | (() => Promise<R>)) => {
            if (!mountedRef.current) return { data: undefined as unknown as R, error: undefined as Error | undefined }
            setLoading(true)
            setError(null)
            try {
                const result = typeof task === 'function' ? await (task as () => Promise<R>)() : await task
 ...
 */
export function useAsyncCallback<TArgs extends any[] = any[], TResult = unknown>(
    ...
    const call = useCallback(
        async (...args: TArgs): Promise<{ data?: TResult; error?: Error } | void> => {
