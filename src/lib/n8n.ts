...
export const N8N_BASE_URL = (import.meta as any)?.env?.VITE_N8N_BASE_URL || 'https://n8n.srv856940.hstgr.cloud'
export const N8N_DEFAULT_WEBHOOK_PATH = (import.meta as any)?.env?.VITE_N8N_WEBHOOK_PATH || 'content-saas'
...
export async function postToN8n(
    ...
    const headers: Record<string, string> = {
        ...
        'X-Env': (import.meta as any)?.env?.MODE || (typeof process !== 'undefined' ? (process.env.NODE_ENV || 'production') : 'production'),
        'X-Request-Id': (globalThis as any)?.crypto?.randomUUID ? (globalThis as any).crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2)}`,
        ...options.headers,
    }
